...
访问 http://localhost:5000（默认 admin/admin123）

| 页面 | 功能 |
|------|------|
| **概览** | 实时状态、余额、持仓、交易、盈亏图表 |
| **交易** | 快速下单表单（仅 local 模式） |
| **策略** | 查看/重载策略配置 |
| **回测** | 选择策略、天数、初始资金，生成回测报告与资产曲线 |
| **设置** | 切换交易所、通知开关、保存配置 |
| **日志** | 查看系统日志（提示使用 `docker logs`） |
...
- `GET /api/trades?limit=20` - 交易记录
- `POST /api/order` - 下单（local）
- `GET /api/strategies` - 可用策略列表
- `POST /api/strategy/reload` - 重载策略配置（无需重启）
- `POST /api/backtest` - 运行回测（返回 JSON 报告）
- `GET /api/logs` - 日志（占位）
...
- 每日自动分析策略表现（胜率、盈亏、回撤）
- 发送学习报告通知（QQ）
- 提供参数调整建议（RSI/MA/布林带/MACD）

---

## 🎯 参考 freqtrade 设计

本系统借鉴了 freqtrade 的功能划分：
- 策略与执行分离
- 多策略支持与配置化
- 回测引擎
- 数据抽象层（可扩展）
...
未来可能增加：命令行界面、Webhook 通知、机器学习优化等。
